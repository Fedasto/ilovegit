...
for j in range(len(sigma)):
    ...
    ks_test_m_f.append(stats.kstest(mass,f(mass))[0]) # I'm interestend on the statistics value and not on the p-value
    ks_test_m_mirr.append(stats.kstest(mass,M_irr(mass,chi))[0])
    
plt.figure(figsize=[8,8])    
plt.plot(sigma,ks_test_m_f, label='KS$(M, f)$')
plt.plot(sigma,ks_test_m_mirr, label='KS$(M_{\\rm irr}, M)$')
