...
plt.plot(x_grid, norm(loc=1,scale=0.002).pdf(x_grid), '-', c='red')
...
PDFtophat = kde_sklearn(f,bandwidth=0.05,kernel="gaussian") # How do I set the best bandwith
plt.plot(xgrid,PDFtophat, label='tophat + gaussian') #Complete

PDFtophat = kde_sklearn(f,bandwidth=0.05,kernel="epanechnikov") #Complete
