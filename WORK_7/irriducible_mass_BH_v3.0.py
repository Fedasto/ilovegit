...
for j in range(len(sigma)):
    m = np.random.normal(loc=1, scale=sigma[j], size=N)
    ks_test_m_f.append(stats.kstest(f(m),M_irr(m,chi))[0]) # I'm interestend on the statistics value and not on the p-value
    ks_test_m_mirr.append(stats.kstest(m,M_irr(m,chi))[0])
    
    ks_test_m_f_2.append(stats.ks_2samp(f(m),M_irr(m,chi))[0]) # I'm trying another ks statistics method
    ks_test_m_mirr_2.append(stats.ks_2samp(m,M_irr(m,chi))[0])
...
def limits(scale):
    ...
    plt.hist(M_irr(mass,chi), bins=50, histtype='step', color='black', density=True, label='irriducible mass')
    plt.hist(F, density=True, histtype='step', label='f',color='orange')
    plt.hist(mass, density=True ,histtype='step',label='Mass', color='blue')
...
scales=[0.001,0.02,1.0]


for j in scales: # this take a while
    limits(j)
    print(j)
